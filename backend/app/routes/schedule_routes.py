...
def _build_schedule_context(user_id, target_date=None):
    ...
    from backend.app.repositories.task_repository import fetch_all_session_dates
    ...
    schedule = generate_schedule(user_id, target_date=target_date)

    for item in schedule:
        item["start_time"] = ensure_ist(item.get("start_time"))
        item["end_time"] = ensure_ist(item.get("end_time"))

    # Fetch all distinct dates that have sessions scheduled
    scheduled_dates = fetch_all_session_dates(user_id)
