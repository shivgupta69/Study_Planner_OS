...
def generate_schedule(user_id, target_date=None):
    """Build the visual day timeline for a specific date."""
    from backend.app.repositories.task_repository import fetch_sessions_by_user_and_date
    ...
    sessions = fetch_sessions_by_user_and_date(user_id, target_date)

    for row in sessions:
        duration = float(row_get(row, "allocated_hours"))
        start_time = current_time
        end_time = start_time + timedelta(hours=duration)

        schedule.append(
            {
                "task": row_get(row, "task_name"),
...
def schedule_user_tasks(user_id, daily_available_hours=DEFAULT_DAILY_AVAILABLE_HOURS):
    ...
    from backend.app.repositories.task_repository import replace_user_sessions
    ...
    # Persist split sessions to the dedicated table
    replace_user_sessions(user_id, schedule_result.get("sessions", []))

    # Also update the main tasks table for backward compatibility / simple views
